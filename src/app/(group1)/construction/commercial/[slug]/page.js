...
export async function generateStaticParams() {
  ...
  if (!res.ok) {
    ...
    console.error("Failed to fetch commercial projects");
    return [];
  }

  const data = await res.json();
  if (!data || !data.data) {
    console.error("Invalid data structure for commercial projects");
    return [];
  }
...
export default async function ProjectSingle({ params }) {
  ...
  // Fetch only projects in Commercial category
  const res = await fetch(
