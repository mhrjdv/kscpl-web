...
export default async function ProjectSingle({ params }) {
  ...
  const res = await fetch(
    "https://kscplcms.cubeone.in/api/projects?filters[projectDetails][Category][$eq]=Institutional&populate=projectDetails.MainImage&populate=projectDetails.Images",
    { next: { revalidate: 60 } }
  );
  if (!res.ok) {
