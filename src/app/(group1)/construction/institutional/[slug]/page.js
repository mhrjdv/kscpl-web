...
export async function generateStaticParams() {
  ...
  const res = await fetch(
    "https://kscplcms.cubeone.in/api/projects?filters[projectDetails][Category][$eq]=Institutional&populate=projectDetails.MainImage"
  );

  if (!res.ok) {
    // In a real app you might do something else; you can also just let it throw
    console.error("Failed to fetch Institutional projects");
    return [];
  }

  const data = await res.json();
  if (!data || !data.data) {
    console.error("Invalid data structure for Institutional projects");
    return [];
  }
...
export default async function ProjectSingle({ params }) {
  ...
  // Fetch only projects in Institutional category
  const res = await fetch(
    "https://kscplcms.cubeone.in/api/projects?filters[projectDetails][Category][$eq]=Institutional&populate=projectDetails.MainImage&populate=projectDetails.Images&populate=projectDetails.brochure",
    { next: { revalidate: 60 } }
  );
  if (!res.ok) {
