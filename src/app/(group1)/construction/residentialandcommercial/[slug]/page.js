...
export default async function ProjectSingle({ params }) {
  ...
  const res = await fetch(
    "https://kscplcms.cubeone.in/api/projects?filters[projectDetails][Category][$eq]=ResidentialandCommercial&populate=projectDetails.MainImage&populate=projectDetails.Images&populate=projectDetails.brochure",
    { next: { revalidate: 60 } }
  );
  if (!res.ok) {
