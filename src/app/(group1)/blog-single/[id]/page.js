...
import { fetchSingleBlog, fetchBlogs, getRelatedPosts } from '@/lib/utils/api'
...
export async function generateMetadata({ params }) {
  try {
    const blogData = await fetchSingleBlog(params.id);
    return {
  ...
  try {
    ...
    const renderContent = (content, index) => {
      switch (content.type) {
        ...
        case 'heading':
          return (
            <h3 key={index} className='text-3xl 2sm:text-4xl font-bold text-primary-foreground leading-135 pt-[23px] mb-[14px]'>
              {content.children?.[0]?.text}
            </h3>
          );
        case 'list':
    ...
    return (
      <>
        <section>
          <div className='object-cover bg-no-repeat 2xl:pt-[448px] xl:pt-[300px] lg:pt-[200px] pt-[150px] pb-[68px] relative z-[1] after:contents-[""] after:z-[-1] after:absolute after:left-0 after:bottom-0 after:w-full after:h-full after:bg-bottom-liner' 
          ...
          <div className='container lg:pt-30 2sm:pt-20 pt-14'>
            ...
            {relatedPosts.length > 0 && (
              <div>
                ...
                <BlogSlider data={relatedPosts.map(post => ({
                  id: post.id,
